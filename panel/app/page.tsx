...
function hypothesisReviewClass(status: string) {
  ...
  if (status === "proposed" || status === "under_review") return "status status-warning";
  return "status status-active";
}

function HypothesisStatuses({ hypothesis, runs }: { hypothesis: Hypothesis; runs: Run[] }) {
  const testRun = runs.find((run) => run.run_key === `${hypothesis.hypothesis_key}:g0:development`);
